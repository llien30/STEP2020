...
void reverse_heap_add_to_free_list(simple_metadata_t *metadata)
{
    assert(!metadata->next);
    size_t size = metadata->size;
    simple_metadata_t *tmp_metadata = simple_heap.free_head;
    simple_metadata_t *prev_metadata = NULL;
    while (tmp_metadata && tmp_metadata->size > size)
    {
        prev_metadata = tmp_metadata;
        tmp_metadata = tmp_metadata->next;
    }
    // the new metadata size is smaller than the existing
    if (!tmp_metadata)
    {
    ...
    // the new metadata size is bigger than the existing
    if (!prev_metadata)
    {
        simple_heap.free_head = metadata;
        metadata->next = tmp_metadata;
        return;
    }
    prev_metadata->next = metadata;
    metadata->next = tmp_metadata;
...
void *my_malloc(size_t size)
{
    ...
    if (metadata->size < size)
    {
        ...
        metadata->next = NULL;
        // Add the memory region to the free list.
        reverse_heap_add_to_free_list(metadata);
    ...
    if (remaining_size > sizeof(simple_metadata_t))
    {
        ...
        new_metadata->next = NULL;
        // Add the remaining free slot to the free list.
        reverse_heap_add_to_free_list(new_metadata);
    }
    return ptr;
}
